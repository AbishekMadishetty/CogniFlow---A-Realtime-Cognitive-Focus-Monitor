...
class CogniFlowEngine:
    ...
    def __init__(self, shutdown_event):
        ...
        # ── Cloud Telemetry (Phase 1) ──
        self.telemetry_queue = queue.Queue()
        threading.Thread(target=self._telemetry_worker, daemon=True).start()

    def _load_ml_models(self):
        """Loads the Random Forest model and encoders if they exist."""
        try:
            if os.path.exists('cogniflow_rf_model.pkl'):
                self.rf_model    = joblib.load('cogniflow_rf_model.pkl')
                self.le_state    = joblib.load('le_state.pkl')
                self.le_activity = joblib.load('le_activity.pkl')
                print("[Engine] ✓ ML Validation Layer loaded.")
            else:
                print("[Engine] ℹ ML models not found. Running rule-based only.")
        except Exception as e:
            print(f"[Engine] ⚠️ Error loading ML models: {e}")

    def _stream_telemetry(self, score, label, state, ear, variance, yawns, eye_closes, activity, ml_label):
        """Packages the current focus data into a JSON payload and adds it to the queue."""
        payload = {
    ...
    def _telemetry_worker(self):
        """Background thread that safely transmits data to the cloud API."""
    ...
    def _calibrate(self, ear, variance):
        ...
        if elapsed >= CALIBRATION_SECONDS and len(self._cal_samples) >= 5:
            ...
            print(f"[Engine] ✓ Calibrated | "
                  f"EAR={self.baseline_ear:.3f}  VAR={self.baseline_var:.4f}")
    ...
    def _calculate(self, ear, variance, yawn_count, state, mouse_active, is_dark_room, posture):
        raw_scores = {
        ...
        # --- NEW: Dynamic Lighting Weights ---
        active_weights = WEIGHTS.copy()
        if is_dark_room:
            active_weights["ear"]    = 0.45
            active_weights["typing"] = 0.20
            active_weights["state"]  = 0.15
            
        raw = sum(raw_scores[k] * active_weights[k] for k in raw_scores)
        
        # --- NEW: Ergonomics Penalty ---
        if posture in ["SLOUCHING", "LEANING_FORWARD"]:
            raw = raw * 0.90
        ...
        color    = self._color(smoothed)
        return smoothed, label, color, raw_scores
    ...
    def _check_alerts(self, score, yawn_count, eye_close_count,
                      eyes_closed_now, cam_blocked, posture): # <-- Added posture
        now = time.time()

        if cam_blocked:
            if now - self._last_alert_at > 10:
                self.alert_message = (
                    "⚠️  Webcam is being blocked!\n"
        ...
        if posture in ["SLOUCHING", "LEANING_FORWARD"]:
            self.alert_message = (
                ...
                "Check your ergonomics to prevent strain."
            )
            self._last_alert_at = now
            return
    ...
    def run(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            # --- NEW: Fetching Advanced Biometrics ---
            is_dark_room = self.webcam.get_dark_room()
            posture      = self.webcam.get_posture()
            bpm          = self.webcam.get_bpm()
            ...
            # Pause scoring if blocked
            if cam_blocked:
                self.current_label = "Cam Blocked ⚠️"
                self.current_color = "#B71C1C"
                self._check_alerts(0, yawns, eye_closes, False, True, "GOOD_POSTURE")
                print("  🚫 Camera blocked — tracking paused.")
                time.sleep(2)
                continue

           # --- NEW: Pass Dark Room and Posture into Calculator ---
            score, label, color, breakdown = self._calculate(
                ear, variance, yawns, state, active, is_dark_room, posture)

            # ── ML Prediction Layer ──
            if self.rf_model and self.le_state and self.le_activity:
                try:
                    s_idx = self.le_state.transform([state])[0] if state in self.le_state.classes_ else 0
                    a_idx = self.le_activity.transform([activity])[0] if activity in self.le_activity.classes_ else 0
            ...
            # Alerts
            self._check_alerts(score, yawns, eye_closes,
                               eyes_closed, cam_blocked, posture)
           ...
           # Terminal output
            ...
            bpm_display = f"{bpm} BPM" if bpm else "Calc HR..."
            posture_str = "SLOUCH" if posture != "GOOD_POSTURE" else "GOOD_POS"
            
            # --- NEW: Output shows ML, Heart Rate, and Posture inline! ---
            print(f"  [{bar}] {score:3d}  Rule: {label:<10} ML: {self.ml_label:<10} "
                  f"EAR:{eyes_status}  {bpm_display:<10} {posture_str:<10} {activity}")
